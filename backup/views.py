...
def create_backup_ajax(request):
    ...
    try:
        ...
        result = subprocess.run(
            [sys.executable, "manage.py", "create_backup", "--type", backup_type],
            capture_output=True,
            text=True,
...
def delete_backup(request, filename):
    ...
    config = BackupConfiguration.objects.first()
    if not config:
        return JsonResponse({"success": False, "message": "Configuration introuvable"})

    # Sécurité
    if ".." in filename or filename.startswith("/"):
        return JsonResponse({"success": False, "message": "Nom de fichier invalide"})
