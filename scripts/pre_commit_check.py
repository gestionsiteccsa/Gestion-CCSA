...
class PreCommitChecker:
    ...
    def _run_command(
        self, command, capture_output: bool = True, timeout: int = 60, shell: bool = False
    ) -> Tuple[int, str, str]:
    ...
    def check_linter(self) -> CheckResult:
        ...
        if not self.check_tool_installed("flake8"):
            if not self.install_tool("flake8"):
                return CheckResult("Linter", False, "Flake8 non installé", can_fix=False)
        ...
        details = [f"{error_count} erreur(s) de style détectée(s)", "Exemples d'erreurs :"]
        ...
        return CheckResult("Linter", False, f"{error_count} erreurs de style", details=details)
    ...
    def fix_linter(self) -> bool:
        ...
        if self.check_tool_installed("black"):
            ...
            returncode, _, _ = self._run_command(
                ["black", ".", "--exclude", "/(env|venv|__pycache__|migrations|scripts|logs)/"]
            )
            if returncode == 0:
    ...
    def check_tests(self) -> CheckResult:
        ...
        cmd = (
            ["python", "manage.py", "test", test_module, "--verbosity=1"]
            if test_module
            else ["python", "manage.py", "test", "--verbosity=1"]
        )
        ...
        return CheckResult("Tests", False, "Certains tests ont échoué", details=details[:10])
    ...
    def check_django(self) -> CheckResult:
        ...
        if returncode != 0:
            return CheckResult(
                "Django", False, "Erreurs dans la configuration", details=[stderr or stdout]
            )
        ...
        if self.config["checks"]["django"].get("check_deploy", False):
            ...
            if returncode != 0:
                ...
                return CheckResult(
                    "Django", False, "Problèmes de configuration production", details=warnings[:5]
                )
                ...
                if self.config["checks"]["django"].get("auto_migrate", False):
                    if self._confirm("   Des migrations sont manquantes. Les créer ?"):
                        ...
                        if returncode == 0:
                            ...
                            return CheckResult(
                                "Django", True, "Configuration OK, migrations créées"
                            )
    ...
    def check_security(self) -> CheckResult:
        ...
        if "bandit" in self.config["checks"]["security"]["tools"]:
            ...
            if self.check_tool_installed("bandit"):
                exclude = ",".join(
                    ["./tests", "./migrations", "./env", "./venv", "./.git", "__pycache__"]
                )
                returncode, stdout, stderr = self._run_command(
            ...
            if self.check_tool_installed("safety"):
                ...
                if returncode != 0:
                    ...
                    try:
                        ...
                        details = [
                            f"{v.get('package', 'N/A')}: {v.get('vulnerability', 'N/A')}"
                    ...
                    results.append(
                        ("Safety", False, "Vulnérabilités dans les dépendances", details)
                    )
                else:
        ...
        if failed:
            ...
            return CheckResult("Sécurité", False, "Problèmes de sécurité détectés", details=details)
    ...
    def check_files(self) -> CheckResult:
        ...
        if found:
            ...
            if dangerous:
                return CheckResult(
                    "Fichiers", False, "Fichiers sensibles commités!", details=dangerous
                )
            else:
        ...
        if self.config["checks"]["files"].get("check_gitignore", True):
            ...
            if not Path(".gitignore").exists():
                return CheckResult("Fichiers", False, "Fichier .gitignore manquant")
    ...
    def check_git(self) -> CheckResult:
        ...
        if self.config["checks"]["git"].get("conventional_commits", True):
            ...
            allowed_types = self.config["checks"]["git"].get(
        ...
        for i, (name, check_func, fix_func) in enumerate(checks, 1):
            ...
            if result.passed:
                    ...
                    if self._confirm(f"   Corriger automatiquement ?"):
                        if fix_func():
                            ...
                            if result.passed:
                                print(
                                    self.colors.success(
                                        f"   [OK] Correction réussie - {result.message}"
                                    )
                                )
                                ...
                                print(
                                    self.colors.error(
                                        f"   [X] Correction insuffisante - {result.message}"
                                    )
                                )
                ...
                if self.strict_mode:
                    ...
                    print(
                        f"{self.colors.RED}Corrigez les erreurs ci-dessus avant de recommencer.{self.colors.RESET}"
                    )
    ...
    def _print_summary(self, success: bool):
        ...
        if success:
            ...
            print(
                self.colors.GREEN + "Vous pouvez committer en toute sécurité." + self.colors.RESET
            )
        else:
...
Exemples:
    ...
    parser.add_argument(
        "--config",
        "-c",
        default="scripts/pre-commit-config.json",
        help="Chemin vers le fichier de configuration",
    )
    parser.add_argument(
        "--fix", "-f", action="store_true", help="Activer les corrections automatiques"
    )
    parser.add_argument(
        "--yes",
        "-y",
    ...
    parser.add_argument("--from-hook", action="store_true", help="Mode hook git (non-interactif)")
    parser.add_argument(
        "--only", type=str, help="Vérifications spécifiques uniquement (séparées par virgule)"
    )
