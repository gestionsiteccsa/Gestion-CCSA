...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            ...
            type=int,
            default=15,
            help="Nombre de jours après lesquels les notifications sont supprimées (défaut: 15)",
        )
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(
            self.style.SUCCESS(
                f"{deleted_count} notification(s) de plus de {days} jours ont été supprimées."
            )
        )
